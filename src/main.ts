...
function populateUIFromCache(cache: GameDataCache): void {
  ...
  if (htmlGalleryContainer) {
    ...
    if (cache.galleryUrls.length > 0) {
      ...
      htmlGalleryContainer.innerHTML =
...
async function fetchGameDataAndUpdatePage(): Promise<void> {
  ...
  try {
    ...
    if (!gameResponse.ok) {
      throw new Error(
        `http error fetching game data! status: ${gameResponse.status}`,
      );
    }
    ...
    if (gameResult.data && gameResult.data.length > 0) {
      ...
      cacheData.gameDetails.creator.isRNVAccount =
        gameData.creator.isRNVAccount;
      ...
      // handle no game data found
      detailsContainer.innerHTML =
        '<p class="text-danger">failed to load game data.</p>';
      if (galleryContainer)
        galleryContainer.innerHTML =
          '<p class="text-muted">could not load thumbnails.</p>';
    ...
    if (galleryContainer)
      galleryContainer.innerHTML =
        '<p class="text-danger">could not load thumbnails due to an error.</p>';
        ...
        if (htmlGalleryContainer) {
          ...
          const galleryImagePromises = imageItems.map(async (item, index) => {
            ...
            if (imageUrl) {
              ...
              img.src = imageUrl;
              const altText =
                item.altText || `${gameName} thumbnail ${index + 1}`;
              img.alt = altText;
              img.classList.add("img-fluid", "rounded25", "me-2", "mb-2");
...
async function getImageUrlFromAssetDelivery(
  imageId: number,
): Promise<string | null> {
